    ...
    <div ref={containerRef} className="relative w-full max-w-[380px]">
      ...
      <button
        ...
        className={`flex items-center gap-2 h-[42px] px-4 pr-3 rounded-full border transition-all w-full text-left ${
          selectedCommunityId
            ...
            : 'border-white/20 bg-[#1A1D23] text-gray-400'
      ...
      >
        {selectedCommunity ? (
          ...
          <>
            <IoPeopleCircleOutline size={22} className="text-gray-500 shrink-0" />
      ...
      {/* Dropdown Panel */}
      {open && (
        <div className="absolute top-[48px] left-0 w-[380px] bg-[#1A1D23] border border-white/15 rounded-[12px] shadow-2xl z-50 overflow-hidden">
          {/* Header */}
          <div className="flex items-center justify-between px-4 py-3 border-b border-white/10">
            <span className="text-[14px] font-bold text-white">Post to</span>
            <button
              type="button"
          ...
          <div className="px-3 py-2 border-b border-white/10">
            <div className="flex items-center gap-2 bg-white/5 rounded-full px-3 h-[36px]">
              ...
              <input
                ...
                className="bg-transparent flex-1 text-[13px] text-white placeholder-gray-500 focus:outline-none"
              />
              {search && (
          ...
          <div className="overflow-y-auto max-h-[360px]">
            {loading ? (
              ...
              <p className="text-center text-gray-500 text-[13px] py-8">No communities found</p>
            ) : (
              <>
                {joined.length > 0 && (
                  <div>
                    <p className="px-4 pt-3 pb-1 text-[10px] font-bold text-gray-500 uppercase tracking-wider">
                      Your Communities
                    </p>
...
const CommunityOption: React.FC<{
  ...
  >
    ...
    <div className="flex flex-col min-w-0 flex-1">
      <span className="text-[13px] font-semibold text-white truncate">r/{community.id}</span>
